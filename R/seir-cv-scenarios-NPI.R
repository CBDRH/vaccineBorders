...
# - R0 (3.5/7.0)
# Interventions come in to place as a 70% reduction once active cases exceed 100
...
simulation_results_NPI <- vector(mode="list",length=num_draws*num_sims)
names(simulation_results_NPI) <- paste0(rep(sim_ids, each = num_draws), '_', 1:num_draws)
...
intC <- covoid::reactive_intervention(threshold = 10000,
                                         reduce = 0.3,
...
for(i in 1:num_sims){
    ...
    param_i$contact_intervention <- intC

    for(j in 1:num_draws){

        id_i_n <-paste0(sim_ids[i], '_', j)

        simulation_results_NPI[[id_i_n]] <- simulate_seir_cv(t = NDAYS,
                                                         state_t0 = state_i,
                                                         param = param_i)
    }
}
...
incidence_results_NPI <- imap(simulation_results_NPI, compileIncidence2)

# Compile simulation results as a dataframe
incidence_results_full_NPI <- do.call(rbind,incidence_results_NPI) %>%
    mutate(
...
# Save the data
save(simulation_results_NPI, file = here::here("Outputs/simulation_results_NPI.Rda"))
save(incidence_results_NPI, file = here::here("Outputs/incidence_results_NPI.Rda"))
save(incidence_results_full_NPI, file = here::here("Outputs/incidence_results_full_NPI.Rda"))
